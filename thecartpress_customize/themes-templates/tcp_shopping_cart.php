...
if ( $source->see_address() ) : ?>
	...
	<?php else : ?>
		<table id="shipping_billing_info" width="100%" cellpading="0" cellspacing="0">
			<tr valign="top">
				<th class="billing_info" style="text-align: left"><h3><?php _e( 'Contact Information', 'tcp' ); ?></h3></th>
			</tr>
			<tr valign="top">
				<td class="billing_info">
					<?php echo $source->get_billing_firstname();?> <?php echo $source->get_billing_lastname(); ?>
...
global $pagenow;
if ($pagenow=='admin.php' or $pagenow=='printorder.php')
{
...
if ( $source->has_order_details() ) :
	$i = 0;
	global $wpdb;
	$xvorder_id = $source->get_order_id();
		$xvTBName = $wpdb->prefix . 'tcp_quote_data';
	$xvquote = $wpdb->get_row("SELECT * FROM $xvTBName WHERE order_id = '{$xvorder_id}' ");
	...
	<tr class="tcp_cart_product_row <?php if ( $i++ & 1 == 1 ) : ?> par<?php endif; ?>">
		...
		<td class="tcp_cart_price"><?php echo tcp_format_the_price( $xvquote[$order_detail->detail->post_id]['price'] ); ?>
		...
		$price = $xvquote[$order_detail->detail->post_id]['price'] - $discount;
		$price = round( $price, $decimals );
		$tax = $xvquote[$order_detail->detail->post_id]['tax'];
