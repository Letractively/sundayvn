...
{		
	...
	function dealSave()
	{
		...
		// tin.doan edit save dealtype
			$modelType = JModel::getInstance('dealType','enmasseModel');
				//delete all dealtype of this deal
			$deleteDealTypes = $modelType->delete($data['id']);
				//save all dealtypes
			for($i = 0 ; $i < count($data['idType']); $i++)
			{
				$dataType['id'] = NULL;
				$dataType['name'] = $data['txtTypeName'][$i];
				$dataType['price'] = $data['txtPrice'][$i];
				$dataType['origin_price'] = $data['txtOriginalPrice'][$i];
				$dataType['deal_id'] = $row->id;
				$dataType['status'] = $data['type-apply'][$i];
				$dataType['code'] = $data['txtTypeCode'][$i];
				
				$saveDealType = $modelType->store($dataType);
			}
		if ($row->success) 
		{
