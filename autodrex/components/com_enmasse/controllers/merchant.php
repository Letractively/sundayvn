...
{	
	...
	function update()
	{
		...
		$coupon_security_code = trim(JRequest::getVar('coupon_security_code',NULL,'get'));
		...
		else
		{
			...
			elseif($action=="Used")
			{
				...
				else
				{
					...
					if ( $useSecurityCode )
					{
						if ( $coupon_security_code != $invty->security_code )
						{
							$msg = JText::_( 'COUPON_SECURITY_CODE_INVAILD'). '('.$invty->name.")";
							$this->setRedirect("index.php?option=com_enmasse&controller=merchant&task=dealCouponMgmt&filter[deal_id]=$orderItem->pdt_id", $msg,'warning');
							return;
						}
					}
