...
class EnmasseModelMerchant extends JModel
{
 var $_total = null;
     ...
     {
     	$db = JFactory::getDBO();
     	$query = "SELECT COUNT(*) FROM #__enmasse_merchant_branch";
     	$db->setQuery($query);
     	$count = $db->loadResult();
     	return $count;
     }
	...
	{
		$db = JFactory::getDBO();
		$query = "SELECT * FROM #__enmasse_merchant_branch
		           WHERE published = 1";
		$db->setQuery( $query );
	...
	function deleteList($cids)
	{
		...
		foreach($cids as $cid)
		{
			...
			//remove merchant group for this user from joomla user group table
	...
	function getMerchantByName($merchantName)
	{
		$db 	= JFactory::getDBO();
		$query = ' SELECT 
		              * 
	...
	function getMerchantBySaleId($saleId)
	{
		$db 	= JFactory::getDBO();
		$query = " SELECT 
		              * 
		           FROM 
		              #__enmasse_merchant_branch
		           WHERE 
		            sales_person_id = $saleId";
		$db->setQuery($query);
		$merchants = $db->loadObjectList('id');
		return $merchants;
	}
	...
	{
		$db = JFactory::getDBO();
		$query = "	UPDATE #__enmasse_merchant_branch 
						SET
