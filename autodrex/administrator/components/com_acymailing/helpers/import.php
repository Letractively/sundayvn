...
class importHelper{
	...
	function zohocrm($action = ''){
		...
		$listClass= acymailing_get('class.list');
		$subscriberClass = acymailing_get('class.subscriber');
		$tableFields = $db->getTableFields(acymailing_table('subscriber'));
		$tableInfos = array_keys(reset($tableFields));
		$config =& acymailing_config();
	...
	function sobipro(){
		...
		$db = JFactory::getDBO();
		$query='SELECT fid, nid FROM #__sobipro_field WHERE nid LIKE "field_contact" OR  nid LIKE "field_email"';
		$db->setQuery($query);
		$nidResult = $db->loadObjectList("nid");
		if(empty($nidResult["field_contact"]) OR empty($nidResult["field_email"])) return false;
		$time = time();
		$query = 'INSERT IGNORE INTO '.acymailing_table('subscriber').' (`email`,`name`,`confirmed`,`created`,`enabled`,`accept`,`html`) SELECT b.baseData AS email, a.baseData AS name, 1 as confirmed, '.$time.' as created, 1 as enabled, 1 as accept, 1 as html FROM #__sobipro_field_data AS a LEFT JOIN #__sobipro_field_data AS b ON a.sid=b.sid WHERE a.`fid` = '.$nidResult["field_contact"]->fid.' AND b.`fid` = '.$nidResult["field_email"]->fid.' AND b.baseData LIKE "%@%" AND b.baseData IS NOT NULL AND a.baseData IS NOT NULL ORDER by a.sid ';
		$db->setQuery($query);
		$db->query();
		$insertedUsers = $db->getAffectedRows();
		$app->enqueueMessage(JText::sprintf('IMPORT_NEW',$insertedUsers));
