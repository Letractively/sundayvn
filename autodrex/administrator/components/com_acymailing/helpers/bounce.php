...
class bounceHelper{
	...
	function handleMessages(){
		...
		while(($msgNB>0) && ($this->_message = $this->getMessage($msgNB))){
			...
			if(empty($this->_message->mailid) && !empty($this->_message->subid)){
				$this->db->setQuery('SELECT `mailid` FROM '.acymailing_table('userstats').' WHERE `subid` = '.(int) $this->_message->subid.' AND `sent` = 1 ORDER BY `senddate` DESC LIMIT 1');
	...
	function _actionuser(&$oneRule){
		...
		if(!empty($oneRule->action_user['min']) && $oneRule->action_user['min']>1){
			$this->db->setQuery('SELECT COUNT(mailid) FROM #__acymailing_userstats WHERE bounce > 0 AND subid = '.$this->_message->subid);
	...
	function _actionmessage(&$oneRule){
		...
		if(isset($oneRule->action_message['save']) && !empty($this->_message->subid) && empty($oneRule->action_user['delete'])){
			$data = array();
			$data[] = 'SUBJECT::'.htmlentities($this->_message->subject);
