...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return ((Yii::app()->getModule('user')->isAdmin())?array(
			...
			array('firstname, lastname, username, email, createtime, lastvisit, superuser, status', 'required'),
			array('createtime, lastvisit, superuser, status', 'numerical', 'integerOnly'=>true),
		):((Yii::app()->user->id==$this->id)?array(
			array('firstname, lastname, username, email', 'required'),
			array('username', 'length', 'max'=>20, 'min' => 3,'message' => UserModule::t("Incorrect username (length between 3 and 20 characters).")),
			array('email', 'email'),
			array('username', 'unique', 'message' => UserModule::t("This user's name already exists.")),
			array('username', 'match', 'pattern' => '/^[A-Za-z0-9_]+$/u','message' => UserModule::t("Incorrect symbols (A-z0-9).")),
	...
	public function scopes()
    {
        return array(
            ...
            'notsafe'=>array(
            	'select' => 'id, firstname, lastname, username, password, email, activkey, createtime, lastvisit, superuser, status',
            ),
        );
    }
    ...
    {
        return array(
            'select' => 'id, firstname, lastname, username, email, createtime, lastvisit, superuser, status',
        );
    }
	...
	public function save(){
		...
		if( $is_new ){
			$phoneid = Helpers::generate_phone_id();
			$user_id = $this->id;
			
			// insert to tbl_user_phone
