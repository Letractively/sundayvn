...
class plgSystemJqueryeasy extends JPlugin {
	...
	function onAfterRoute() {
		...
		if ($this->_usejQueryUI) { // use jQuery UI
			...
			if ($jQueryUIVersion == 'local') {
				...
				$this->_jquipath = $jQueryHTTP."://ajax.googleapis.com/ajax/libs/jqueryui/".$jQueryUIVersion."/jquery-ui".$jQueryCompressed.".js";
			}			
			...
			if ($jQueryUITheme != 'none') {				
				if ($jQueryUITheme == 'custom') {
					...
					if ($customCSS) {
						...
						$this->_jquicsspath = $jQueryHTTP."://ajax.googleapis.com/ajax/libs/jqueryui/".$jQueryUIVersion."/themes/base/jquery-ui.css";
					}
				} else {
					$this->_jquicsspath = $jQueryHTTP."://ajax.googleapis.com/ajax/libs/jqueryui/".$jQueryUIVersion."/themes/".$jQueryUITheme."/jquery-ui.css";
				}
	...
	function onBeforeRender() {
		...
		// + on its own with caption.js when wanting to eliminate the caption functionality (also eliminate JCaption script declaration) and there is no other library loaded
		// + in frontend
		// + user is guest (like that, articles modified through the frontend will still get saved) $user = &JFactory::getUser(); if ($user->get('guest')) {}
		//   or view is not form (editing of articles)
		
		if ($this->params->get('disablemootools', 0) && $app->isSite() && !$needed_library_present && $_GET['view'] != 'form') {
