...
class plgAcymailingVirtuemart extends JPlugin
{
	...
	?>
		...
		<table width="100%" class="adminform">
			<tr>
				...
				<td colspan="2">
				<?php echo JHTML::_('select.radiolist', $contentType, 'contenttype' , 'size="1" onclick="updateTag()"', 'value', 'text', $pageInfo->contenttype); ?>
				</td>
				<td>
	...
	<table width="100%" class="adminform">
		...
		<tr>
			...
			<?php echo JText::_('MAX_ARTICLE'); ?>
			 ...
			 <td>
			 	<input name="max_article" size="10" value="20" onchange="updateTagAuto();"/>
			</td>
			<td>
		...
		<tr>
			...
			<?php echo 'Manufacturer'; ?>
			 ...
			 <td>
			 	...
			 	echo JHTML::_('select.genericlist', $manufacturers, 'manufacturer' , 'size="1" onchange="updateTagAuto();"','manufacturer_id','mf_name'); ?>
			</td>
			<td>
				<?php echo JText::_('FIELD_COLUMNS'); ?>
			 </td>
			 <td>
			 	<select name="cols" onchange="updateTagAuto();" size="1">
	...
	$value= array();
 	...
 	$percent_total = JHTML::_('select.radiolist', $value, 'coupon_percent' , 'onclick="updateTagCoupon();"', 'value', 'text', 'percent|');
 	$value= array();
 	$value[] = JHTML::_('select.option', 'permanent|',JText::_('COUPON_PERMANENT'));
 	$value[] = JHTML::_('select.option', 'gift|',JText::_('COUPON_GIFT'));
 	$permanent = JHTML::_('select.radiolist', $value, 'coupon_permanent' , 'onclick="updateTagCoupon();"', 'value', 'text', 'gift|');
	?>
	<script language="javascript" type="text/javascript">
	<!--
	...
	<tr><td><?php echo JText::_('COUPON_NAME'); ?></td><td><input id="coupon_name" onchange="updateTagCoupon();" type="text" size="30" value="[name][key][value]"></td><td><?php echo $permanent; ?></td></tr>
	<tr><td><?php echo JText::_('COUPON_VALUE'); ?></td><td><input id="coupon_value" onchange="updateTagCoupon();" type="text" size="10" value=""></td><td><?php echo $percent_total; ?></td></tr>
	</table>
	<?php
	echo $tabs->endPanel();
	...
	if(version_compare($this->version,'2.0.0','<' )){
		$tableInfos = $db->getTableFields('#__vm_user_info');
	}else{
		$tableInfos = $db->getTableFields('#__virtuemart_userinfos');
	}
	...
	function generateCoupon(&$allresults,$i,&$user){
		...
		if(version_compare($this->version,'2.0.0','<' )){
			...
			$db->setQuery('INSERT INTO #__virtuemart_coupons (`coupon_code`,`percent_or_total`,`coupon_type`,`coupon_value`) VALUES ('.$db->Quote($name).','.$db->Quote($percent).','.$db->Quote($gift).','.$db->Quote($value).')');
		}
		$db->query();
		return $name;
	}
	function acymailing_generateautonews(&$email){
			...
			foreach($allresults[0] as $i => $oneTag){
				...
				if(!empty($parameter->filter) AND !empty($email->params['lastgenerateddate'])){
					...
					if($parameter->filter == 'modify'){
						$condition .= version_compare($this->version,'2.0.0','<' ) ? ' OR b.`mdate` >\''.$email->params['lastgenerateddate'].'\'' : "b.`modified_on` > '".date( 'Y-m-d H:i:s',$email->params['lastgenerateddate'] - date('Z'))."'";
					}
					$where[] = $condition;
				}
	 ...
	 function _replaceProduct(&$allresults,$i){
		...
		if(version_compare($this->version,'2.0.0','<' )){
			return $this->_getProduct1($tag);
		}else{
			return $this->_getProduct2($tag);
		}
	 }
	 function _getProduct2($tag){
		...
		$link = ACYMAILING_LIVE.'index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id='.$product->virtuemart_product_id.'&virtuemart_category_id='.$product->virtuemart_category_id;
		...
		$price = $currencyHelper->priceDisplay($product->product_price,$product->product_currency);
		...
		if(empty($tag->type) || $tag->type != 'title'){
			if(!empty($product->images)){
				...
				$result .= '<a target="_blank" style="text-decoration:none;border:0px" href="'.$link.'" ><img style="float:left;margin:5px;border:0px" src="'.$mainPict->file_url_thumb.'"/></a>'.$description;
			}else{
				$result .= $description;
			}
		}
	 ...
	 function _getProduct1($tag){
		...
		if(empty($tag->type) || $tag->type != 'title'){
			if(!empty($product->product_thumb_image)){
				...
				$result .= '<a target="_blank" style="text-decoration:none;border:0px" href="'.$link.'" ><img style="float:left;margin:5px;border:0px" src="'.$picturePath.'"/></a>'.$description;
			}else{
				$result .= $description;
			}
		}
	...
	function onAcyDisplayFilters(&$type,$context="massactions"){
		...
		if(version_compare($this->version,'2.0.0','<')){
			...
			$fields = reset($db->getTableFields('#__virtuemart_userinfos'));
		}
		$vmfield = array();
		...
		if(!empty($vmfield)){
			...
			$return .= ' '.$operators->display("filter[__num__][vmfield][operator]").' <input class="inputbox" type="text" name="filter[__num__][vmfield][value]" onchange="countresults(__num__)" size="50" value="" />';
			$return .= '</div>';
	...
	function onAcyProcessFilter_vmallorders(&$query,$filter,$num){
		...
		if(version_compare($this->version,'2.0.0','<')){
			...
			$lj = "`#__virtuemart_orders` as vmallorders$num ON vmallorders$num.`created_by` = sub.`userid`";
		}
		...
		$operator = ($filter['type'] == 'IN') ? 'IS NOT NULL' : 'IS NULL';
	 	if(!empty($filter['payment']) && version_compare($this->version,'2.0.0','<')){
	 		...
	 		if(version_compare($this->version,'2.0.0','<')){
	 			...
	 			$query->where[] = "vmallorders$num.`created_by` ".$operator;
	 		}
	 	}
	}
	...
	function onAcyProcessFilter_vmorder(&$query,$filter,$num){
		...
		if(version_compare($this->version,'2.0.0','<')){
			...
			$allIds  = $query->db->loadResultArray();
			if(empty($allEmails)) $allEmails[] = 'none';
			...
			$lj = "#__virtuemart_order_items as vmorderitem_$num ON vmorderitem_$num.created_by = sub.userid ";
			...
			if(!empty($filter['creationdateinf'])) $lj .= " AND vmorderitem_$num.created_on > ".$query->quote($filter['creationdateinf']);
			if(!empty($filter['creationdatesup'])) $lj .= " AND vmorderitem_$num.created_on < ".$query->quote($filter['creationdatesup']);
			$query->leftjoin['vmorderitem_'.$num] = $lj;
			$query->where[] = "vmorderitem_$num.created_by ".(empty($filter['type']) ? 'IS NULL' : " > 0");
		}
	}
}//endclass