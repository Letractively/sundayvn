...
$xaviDB = JFactory::getDbo();
		...
		if(!empty($formData)){
			...
			$xaviDB->setQuery('select * from #_users where `email` ="'. $formData['email']  .'" ');
			$result = $xaviDB->loadRow();
			if (empty($result))
			{
				$session =& JFactory::getSession();
			$session->set( 'enmasse_new_user' , $formData );
			}
		}
	...
	function optout(){
		...
		if(empty($alreadyExists->subid)){
			...
			if ($ajax){
				...
				echo '{"message":"'.addslashes(JText::sprintf('NOT_IN_LIST','<b><i>'.$email.'</i></b>')).'","type":"error","code":"8"}';
