...
JFactory::getApplication()->setUserState("com_enmasse.checkout.dataautho", $arData);
		...
		if($cart->getTotalPrice() > 0)
		{
			...
			$payGty = JModel::getInstance('payGty','enmasseModel')->getById(4); //Authorize.Net
		...
		if($cart->getTotalPrice() > 0)
		{
			$attributeConfig = json_decode($payGty->attribute_config);
			...
			echo '<form name="checkoutForm" action="' . JURI::base() . "index.php?option=com_enmasse&controller=payment&task=gateway&orderId=" . $order->id . '" method="post">';
			echo '<input type="hidden" name="x_card_num" size="25" value="' . $arData['x_card_num'] . '" />';
			echo '<input type="hidden" name="x_exp_month" size="25" value="' . $arData['x_exp_month'] . '" />';
			echo '<input type="hidden" name="x_exp_year" size="25" value="' . $arData['x_exp_year'] . '" />';
			echo '<input type="hidden" name="x_card_code" size="4"value="' . $arData['x_card_code'] . '" />';
			echo '<input type="hidden" name="x_first_name" size="25" value="' . $arData['x_first_name'] . '" />';
			echo '<input type="hidden" name="x_last_name" size="25" value="' . $arData['x_last_name'] . '" />';
			echo '<input type="hidden" name="x_email" size="25" value="' . $arData['x_email'] . '" />';
			echo '<input type="hidden" name="x_invoice_num" value="' . $order->id . '" >';
			echo '<input type="hidden" name="x_description" value="' . $item->item->short_desc . '">';
			echo '<input type="hidden" value="' . $cart->getTotalPrice() . '" name="x_amount">';
			echo '<input type="hidden" type="text" name="x_type" value="' . $attributeConfig->type .'">';
			echo '<input type="hidden" value="Process" name="x_process" />';
			echo '</form>';
			echo '<script>document.checkoutForm.submit();</script>';

			//if can't submit form
