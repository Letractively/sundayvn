	...
	<?php
	 ...
	 function _replaceContent(&$results,$i){
		...
		if($tag->type != "title"){
			...
			if(version_compare(JVERSION,'1.6.0','>=') && !empty($article->images) && !empty($tag->pict)){
				...
				if(!empty($images->$pictVar)){
					...
					$contentText .= '<img style="'.$style.'" border="0" src="'.JURI::root().$images->$pictVar.'" />';
				}
			}
			...
			$result = '<table cellspacing="0" cellpadding="0" border="0" width="100%"><tr><td><div class="acymailing_content" >'.$result.'</div></td></tr></table>';
		}
		if(!empty($tag->theme)){
