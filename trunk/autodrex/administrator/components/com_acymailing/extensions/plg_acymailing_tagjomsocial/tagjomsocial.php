...
class plgAcymailingTagjomsocial extends JPlugin
{
	...
	function plgAcymailingTagjomsocial(&$subject, $config){
	 ...
	 function acymailingtagjomsocial_show(){
		...
		$db = JFactory::getDBO();
		$tableInfos = $db->getTableFields(acymailing_table('community_users',false));
	...
	function acymailing_replaceusertags(&$email,&$user,$send = true){
		...
		foreach($results as $var => $allresults){
			foreach($allresults[0] as $i => $oneTag){
				...
				$tags[$oneTag] = $mytag->default;
				$mytag->userid = intval($mytag->userid);
				if(empty($mytag->userid)) continue;
				if($mytag->userid != $this->lastuserid){
					$this->lastuserid = $mytag->userid;
					$this->valuesNum = null;
					$this->valuesString = null;
				}
				if(is_numeric($field)){
					if($this->valuesNum === null){
						$db->setQuery('SELECT `field_id`,`value` FROM '.acymailing_table('community_fields_values',false).' WHERE user_id = '.$mytag->userid);
						$this->valuesNum = $db->loadObjectList('field_id');
					}
					if(isset($this->valuesNum[$field]->value)) $tags[$oneTag] = $this->valuesNum[$field]->value;
				}else{
					if($this->valuesString === null){
						$db->setQuery('SELECT * FROM '.acymailing_table('community_users',false).' WHERE userid = '.$mytag->userid.' LIMIT 1');
						$this->valuesString = $db->loadObject();
					}
					if(isset($this->valuesString->$field)) $tags[$oneTag] = $this->valuesString->$field;
					if(in_array($field,array('avatar','thumb'))){
						$tags[$oneTag] = '<img src="'.ACYMAILING_LIVE.$this->valuesString->$field.'"/>';
					}
				}
			}
		}
