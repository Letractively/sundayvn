...
<div class="clear"></div>
	...
	<form method="post" name="frm">
		...
		<tr valign="top">
			...
			<?Php
						global $xavi_cart_source;
							$xvTBName = $wpdb->prefix . 'tcp_quote_data';
		$xvquote = $wpdb->get_row("SELECT * FROM $xvTBName WHERE order_id = '{$order_id}' ");
$xvquote = unserialize($xvquote->data);
					foreach ($xavi_cart_source->orders_details as $xaviOrderItm)
					{
						$xaviTmpPost = get_post($xaviOrderItm->post_id);	
					
			?>
		<tr valign="top">
			<th scope="row">
				<label for="price_input"><?php echo $xaviTmpPost->post_title; ?>:</label>
			</th>
			<td style="line-height: 26px;">
				Price: <input type='text' name="quote[<?Php echo $xaviOrderItm->post_id; ?>][0]" value="<?php echo $xvquote[$xaviOrderItm->post_id]['price'] ?>" />&nbsp; Tax: <input type='text' value="<?php echo $xvquote[$xaviOrderItm->post_id]['tax'] ?>" name="quote[<?Php echo $xaviOrderItm->post_id; ?>][1]" />
			</td>
		</tr>
		<?php
		}	
		...
		<?php
		 ...
		 do_action( 'tcp_admin_order_after_editor', $order_id, $order ); ?>
		</tbody>
		</table>
		<div class="submit">
