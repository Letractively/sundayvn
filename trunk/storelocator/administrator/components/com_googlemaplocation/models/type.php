...
class GoogleMapLocationModelType extends JModelList {
    ...
    function insertType($data) {
        ...
        $query = 'INSERT INTO #__gm_type (type,images) VALUES ("' . $data['type'] . '","' . $_FILES['images']['name'] . '")';
        $db->setQuery($query);
        if (!$db->query()) {
    ...
    function updateType($data, $id) {
        ...
        $query = 'UPDATE #__gm_type SET type = "' . $data['type'] . '",images = "' . $_FILES['images']['name'] . '" where id = ' . (int) $id;
        $db->setQuery($query);
        $db->query();
    }
