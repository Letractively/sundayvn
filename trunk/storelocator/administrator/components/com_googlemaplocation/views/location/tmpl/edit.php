    ...
    <script language="javascript" type="text/javascript">
        ...
        <script>
            ...
            function loadServicesByType(){
                //filter_type: id of select parent 
                    if ($('type') != null) {
                        $('type').addEvent('change', function(){
                        //sub_select: id of sub select parent tag
                        var sub = $('location_service');
                        //URL to load sub select
                        var url = 'index.php?option=com_googlemaplocation&controller=location&task=ajax_service';
                        var filter_type_val = $('type').value;

                        new Request({
                            'url' : url,
                            'data' : {
                                'filter_type_val' : filter_type_val
                            },
                            onSuccess: function(){
                                sub.set('html', arguments[0]);
                            }
                        }).send();
                    });
                }
