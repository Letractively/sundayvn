...
class  FalangControllerHelper  {
	...
	function _setupContentElementCache()
	{
		...
		$db->setQuery( "CREATE TABLE IF NOT EXISTS `#__falang_tableinfo` ( `id` int(11) NOT NULL auto_increment, `joomlatablename` varchar(100) NOT NULL default '',  `tablepkID`  varchar(100) NOT NULL default '', PRIMARY KEY (`id`)) TYPE=MyISAM");
		$db->query();
	...
	public static function _checkDBStructure (){
		...
		if (!isset($data['falangContent'])){
			$sql = "ALTER TABLE `#__falang_content` ADD INDEX `combo` ( `reference_id` , `reference_field` , `reference_table` )" ;
			$db->setQuery($sql);
			$db->query();
