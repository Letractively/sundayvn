...
//sbou falang method
	...
	*/
    ...
    function loadColumn($offset = 0,  $translate=true, $language=null){
        if (!$translate){
            return parent::loadResultArray($offset);
        }
        $results=array();
        $ret=array();
...
//				if ($jfManager->getCfg("transcaching",1)){
	...
	*/
     ...
     * @since 1.5
    */
    static function &getInstance( $driver='mysql', $host='localhost', $user, $pass, $db='', $table_prefix='' )
    {
        $signature = serialize(array($driver, $host, $user, $pass, $db, $table_prefix));
        $database = JDatabase::_getStaticInstance($signature,'JFalangDatabase',true);
