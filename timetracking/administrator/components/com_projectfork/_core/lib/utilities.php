...
class PFform
{
	...
	public function NavButton( $label, $link, $tooltip = NULL, $task = NULL, $section = NULL, $hide = false,$extraCls = '')
	{
		$config = PFconfig::GetInstance();
        ...
        if(!is_null($task) || !is_null($section)) {
            if(!$user->Access($task, $section)) {
                if($hide) return "";
                return '<a class="pf_nav_gray'.$class.' ' . $extraCls . '"'.$title.'><span>'.PFformat::Lang($label).'</span></a>';
            }
        }
		
		return '<a class="pf_nav'.$class.' ' . $extraCls .'"'.$link.$title.'><span>'.PFformat::Lang($label).'</span></a>';
	}
    ...
    {
        $progress = array('0','5', '10', '15', '20', '25', '30', '35', '40', '45', '50',
