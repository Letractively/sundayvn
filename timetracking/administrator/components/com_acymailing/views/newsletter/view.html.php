...
class NewsletterViewNewsletter extends JView
{
	...
	function preview(){
		...
		if($this->type == 'news'){
			...
			if(acymailing_isAllowed($config->get('acl_newsletters_send','all'))) $bar->appendButton( 'Popup', 'acysend', JText::_('SEND'), "index.php?option=com_acymailing&ctrl=send&task=sendready&tmpl=component&mailid=".$mailid);
			JToolBarHelper::divider();
		}
